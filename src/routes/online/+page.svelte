<script lang="ts">
	...
	let gameState: 'home' | 'create' | 'join' | 'browse' | 'lobby' | 'game' | 'complete' | 'reveal' =
		'home';
	let roomCode: string = '';
	...
	let selectedLanguage: keyof typeof languages = 'english';
	...
	// Enhanced language and theme variables
			...
			publicRoomsListener = onValue(
							...
							.map(([code, room]: [string, any]) => {
								...
								if (roomData.isPublic && !roomData.gameStarted) {
									return {
										...
										hostName:
											Object.values(roomData.players || {}).find((p) => p.isHost)?.name || 'Unknown'
									} as PublicRoom;
								}
								return null;
							})
				...
				(error) => {
					...
					if (firebaseError.code === 'permission-denied') {
						alert(
							'Unable to load public rooms. Please check Firebase permissions or try again later.'
						);
						gameState = 'home';
					}
				}
	...
	// Enhanced Generate words with Gemini with model fallback
	async function generateWithGemini(prompt: string): Promise<WordPair> {
		try {
			...
			// Enhanced fallbacks for different languages
			const fallbacks = {
	...
	// Enhanced Start the game with theme support
	async function startGame(): Promise<void> {
		...
		try {
			...
			const prompt =
				gameRoom.settings.language === 'nepali' || gameRoom.settings.language === 'hindi'
					? `Generate two ${languages[gameRoom.settings.language]?.name || 'Nepali'} words for a "Who is the Imposter" game.
...
- Both should be commonly known words in ${languages[gameRoom.settings.language]?.name || 'selected language'}
...
{
  "civilian": "word_from_theme_in_${languages[gameRoom.settings.language]?.name || 'selected_language'}",
  "imposter": "different_but_related_word_from_theme_in_${languages[gameRoom.settings.language]?.name || 'selected_language'}"
}`
					: `Generate two ${languages[gameRoom.settings.language]?.name || 'English'} words for a "Who is the Imposter" game.
...
Respond in JSON format:
{
  ...
  "imposter": "different_but_related_word_from_theme_in_${languages[gameRoom.settings.language]?.name || 'selected_language'}"
			...
			if (gameRoom.settings.includeDumbRole) {
				const availableIndices = Array.from({ length: playerCount }, (_, i) => i).filter(
					(i) => i !== imposterIndex
				);
	...
	$: filteredRooms = publicRooms.filter(
		(room) =>
	...
	$: playersRevealed = gameRoom
		? Object.values(gameRoom.players).filter((p) => p.hasRevealed).length
		: 0;
	$: totalPlayers = gameRoom ? Object.keys(gameRoom.players).length : 0;
	$: imposterPlayer = gameRoom
		? Object.values(gameRoom.players).find((p) => p.role === 'imposter')
		: null;
...
<div class="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
	...
	{#if connectionStatus !== 'connected'}
		<div class="fixed top-4 left-1/2 transform -translate-x-1/2 z-50">
			<div
				class="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-2 rounded-lg shadow-lg flex items-center space-x-2"
			>
	...
	{#if gameState === 'browse'}
		<div class="container mx-auto px-4 py-8 lg:pt-32 md:py-16">
			<div class="max-w-lg mx-auto">
				<div class=" bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-xl p-8">
					<div class="text-center mb-6">
						<div
							class="w-16 h-16 bg-gradient-to-br from-purple-500 to-indigo-600 rounded-2xl mx-auto mb-4 flex items-center justify-center shadow-xl"
						>
							<div class="w-8 h-8 text-white">{@html GlobeIcon}</div>
						</div>
						<h2 class="text-2xl font-black text-gray-900 mb-2">Public Rooms</h2>
						<p class="text-white text-sm">Join an existing public game</p>
					</div>
					...
					<div class="space-y-3 max-h-80 overflow-y-auto mb-6">
						{#if isLoadingRooms}
							<div class="text-center py-8">
								<div
									class="animate-spin rounded-full h-8 w-8 border-2 border-purple-500 border-t-transparent mx-auto mb-2"
								></div>
						...
						{:else}
							{#each filteredRooms as room}
								<div
									class="p-4 bg-gray-50 rounded-xl border border-gray-100 hover:bg-gray-100 transition-all"
								>
									...
									<div class="flex items-center justify-between text-sm text-gray-600 mb-3">
										...
										>
											{languages[room.language]?.name || 'English'}
										</span>
									</div>
	...
	{#if gameState === 'create'}
		<div class="h-full flex flex-col lg:py-32 max-w-6xl mx-auto">
			<div class="text-center mb-4 lg:mb-6">
				<div
					class="inline-flex items-center bg-green-500/20 border border-green-500/30 rounded-full px-3 lg:px-4 py-1 lg:py-2 mb-2 lg:mb-4"
				>
					<div class="w-1.5 h-1.5 lg:w-2 lg:h-2 bg-green-400 rounded-full mr-2 animate-pulse"></div>
					<span class="text-green-200 text-xs lg:text-sm font-medium">Room Setup</span>
				</div>
				<h2 class="text-2xl lg:text-3xl font-black text-white mb-1 lg:mb-2">Create Your Room</h2>
				<p class="text-sm lg:text-lg text-slate-300">Configure your game settings</p>
			</div>
			<div
			...
			>
				<!-- Settings Grid -->
				<div
					class="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6 mb-4 lg:mb-6 flex-1 overflow-y-auto custom-scrollbar"
				>
					<!-- Left Column -->
					<div class="space-y-4 lg:space-y-6">
						<div>
							...
							<input
								...
								bind:value={playerName}
								placeholder="Enter your name..."
								class="w-full p-3 lg:p-4 bg-white/10 border border-white/20 rounded-lg text-white placeholder-slate-400 focus:border-green-400 focus:outline-none text-sm lg:text-base"
							/>
						</div>

						<!-- Room Visibility -->
						<div>
							<label class="block text-sm font-medium text-slate-300 mb-3">Room Type</label>
							<div class="grid grid-cols-2 gap-2 p-1 bg-white/10 rounded-lg">
								<button
									type="button"
									on:click={() => (isPublicRoom = true)}
									class="flex items-center justify-center py-2 lg:py-3 px-3 lg:px-4 rounded-md text-sm font-medium transition-all duration-200 hover:cursor-pointer space-x-2
                                    {isPublicRoom
						...
						{#if isPublicRoom}
							<div>
								<label class="block text-sm font-medium text-slate-300 mb-2">Room Name</label>
								<input
									type="text"
									bind:value={roomName}
					...
					<div class="space-y-4 lg:space-y-6">
						<!-- Max Players -->
						<div>
							<label class="block text-sm font-medium text-zinc-300 mb-2">Max Players</label>
							<select
								bind:value={maxPlayers}
								class="w-full p-3 lg:p-4 bg-white/10 border border-white/20 rounded-lg text-zinc-900 focus:border-green-400 focus:outline-none text-sm lg:text-base"
							>
						...
						<!-- Enhanced Language Selection with Dropdown -->
						<div>
							<label class="block text-sm font-medium text-slate-300 mb-3">🌍 Language</label>
							<div class="relative language-dropdown">
								<button
									type="button"
									on:click={() => (showLanguageDropdown = !showLanguageDropdown)}
									class="w-full p-3 lg:p-4 bg-white/10 border border-white/20 rounded-lg text-white focus:border-green-400 focus:outline-none text-sm lg:text-base flex items-center justify-between hover:cursor-pointer"
								>
									<div class="flex items-center space-x-3">
										<span class="text-xl lg:text-2xl"
											>{languages[selectedLanguage]?.flag || '🇺🇸'}</span
										>
										<span class="font-medium">{languages[selectedLanguage]?.name || 'English'}</span
										>
									</div>
									<div
										class="w-4 h-4 text-slate-400 transform transition-transform {showLanguageDropdown
											? 'rotate-180'
											: ''}"
									>
								...
								{#if showLanguageDropdown}
									<div
										class="absolute z-10 w-full mt-2 bg-slate-800 rounded-lg shadow-xl border border-white/20 max-h-60 overflow-y-auto"
									>
										{#each Object.entries(languages) as [code, lang]}
											<button
												...
												class="w-full p-3 text-left hover:bg-white/10 transition-all flex items-center space-x-3 first:rounded-t-lg last:rounded-b-lg hover:cursor-pointer
                                                {selectedLanguage === code
						...
						<div>
							...
							<div class="space-y-3">
								<div
									class="flex items-center justify-between p-3 bg-white/10 rounded-lg border border-white/20"
								>
									<div>
										<div class="font-medium text-white text-sm">Random Theme</div>
										<div class="text-xs text-slate-400">Let AI choose automatically</div>
									</div>
									<button
										type="button"
										on:click={toggleThemeInput}
										class="relative w-12 h-6 rounded-full transition-all duration-300 focus:outline-none hover:scale-110 hover:cursor-pointer
                                        {!showThemeInput ? 'bg-green-500' : 'bg-white/20'}"
									>
										<div
											class="absolute w-5 h-5 bg-white rounded-full shadow-sm transition-transform duration-300 top-0.5 flex items-center justify-center
                                        {!showThemeInput ? 'translate-x-6' : 'translate-x-0.5'}"
										>
											{#if !showThemeInput}
								...
								{#if showThemeInput}
									<input
										...
										class="w-full p-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-slate-400 focus:border-green-400 focus:outline-none text-sm"
									/>
									<div class="text-xs text-slate-400">
										Examples: {availableThemes[selectedLanguage]?.slice(0, 3).join(', ')}...
									</div>
								{/if}
							</div>
						</div>
						...
						<div>
							...
							>
								<div>
									<div class="font-medium text-white text-sm">🤔 Mystery Player</div>
									<div class="text-xs text-slate-400">Someone gets no word</div>
								</div>
								<button
									type="button"
									on:click={() => (includeDumbRole = !includeDumbRole)}
									class="relative w-12 h-6 rounded-full transition-all duration-300 focus:outline-none hover:scale-110 hover:cursor-pointer
                                    {includeDumbRole ? 'bg-green-500' : 'bg-white/20'}"
								>
									<div
										class="absolute w-5 h-5 bg-white rounded-full shadow-sm transition-transform duration-300 top-0.5 flex items-center justify-center
                                    {includeDumbRole ? 'translate-x-6' : 'translate-x-0.5'}"
									>
										{#if includeDumbRole}
											<div class="text-green-500 text-xs font-bold">?</div>
										{/if}
				...
				<div class="flex gap-3 lg:gap-4 pt-4 border-t border-white/20">
					...
					<button
						on:click={createRoom}
						class="flex-2 bg-gradient-to-r from-green-600 to-teal-600 hover:from-green-500 hover:to-teal-500 text-white font-bold py-2 lg:py-3 px-4 lg:px-6 rounded-lg transition-all text-sm lg:text-base"
					>
						Create Room
					</button>
				</div>
	...
	<!-- Join Room Screen -->
	{#if gameState === 'join'}
		<div class="h-full flex flex-col lg:py-32 max-w-4xl mx-auto">
			<div class="text-center mb-4 lg:mb-6">
				<div
				...
				>
					...
					<span class="text-blue-200 text-xs lg:text-sm font-medium">Join Room</span>
				</div>
				<h2 class="text-2xl lg:text-3xl font-black text-white mb-1 lg:mb-2">Join by Code</h2>
				<p class="text-sm lg:text-lg text-slate-300">Enter a private room code to join</p>
			</div>
			<div
				class="flex-1 bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl lg:rounded-2xl p-4 lg:p-6 flex flex-col min-h-0 max-w-2xl mx-auto w-full"
			>
				<!-- Join Form -->
				<div class="space-y-4 lg:space-y-6 mb-6 flex-1">
					<div>
						<label class="block text-sm font-medium text-slate-300 mb-2">Your Name</label>
						<input
							type="text"
							bind:value={playerName}
							placeholder="Enter your name..."
							class="w-full p-3 lg:p-4 bg-white/10 border border-white/20 rounded-lg text-white placeholder-slate-400 focus:border-blue-400 focus:outline-none text-sm lg:text-base"
						/>
					</div>
					...
					<div>
						...
						<input
							...
							class="w-full p-4 lg:p-6 bg-white/10 border border-white/20 rounded-lg text-white placeholder-slate-400 focus:border-blue-400 focus:outline-none uppercase text-center text-xl lg:text-3xl font-black tracking-widest"
							maxlength="6"
						/>
					</div>
				</div>
				...
				<div class="flex gap-3 lg:gap-4 pt-4 border-t border-white/20">
					...
					<button
						on:click={joinRoom}
						class="flex-2 bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white font-bold py-2 lg:py-3 px-4 lg:px-6 rounded-lg transition-all text-sm lg:text-base"
					>
						Join Room
					</button>
				</div>
	...
	{#if gameState === 'lobby'}
		<div class="h-full flex flex-col lg:py-32 max-w-4xl mx-auto">
			<div class="text-center mb-4 lg:mb-6">
				...
				>
					<div class="w-1.5 h-1.5 lg:w-2 lg:h-2 bg-green-400 rounded-full mr-2 animate-pulse"></div>
					<span class="text-green-200 text-xs lg:text-sm font-medium">Room Lobby</span>
				</div>
				<h2 class="text-2xl lg:text-3xl font-black text-white mb-1 lg:mb-2">
					{gameRoom?.roomName || 'Game Lobby'}
				</h2>
				<div class="flex items-center justify-center space-x-3 mb-2">
					<div class="bg-white/10 px-3 py-1 rounded-lg">
						<span class="text-xs font-medium text-slate-300">Room:</span>
						<span class="text-sm font-black text-white ml-2">{roomCode}</span>
					</div>
					<button
						on:click={copyRoomCode}
						class="p-1.5 bg-white/10 hover:bg-white/20 rounded-lg transition-all hover:cursor-pointer"
						title="Copy room code"
					>
						<div class="w-4 h-4 text-slate-300">{@html CopyIcon}</div>
					</button>
				</div>
				{#if gameRoom?.isPublic}
					<div
						class="inline-flex items-center text-xs text-green-300 bg-green-500/20 px-2 py-1 rounded-full"
					>
				...
				{:else}
					<div
						class="inline-flex items-center text-xs text-slate-300 bg-white/10 px-2 py-1 rounded-full"
					>
			...
			>
				...
				<div class="mb-4 lg:mb-6">
					<h3 class="text-sm lg:text-base font-bold text-white mb-3">
						Players ({totalPlayers}/{gameRoom?.maxPlayers || 8})
					</h3>
					<div class="space-y-2 lg:space-y-3 max-h-48 lg:max-h-64 overflow-y-auto custom-scrollbar">
						{#each Object.values(gameRoom?.players || {}) as player}
							...
							>
								<div class="flex items-center space-x-3">
									<div
										class="w-8 h-8 lg:w-10 lg:h-10 bg-gradient-to-br from-blue-500 to-purple-600 rounded-lg flex items-center justify-center shadow-lg"
									>
									...
									<div>
										<div class="flex items-center space-x-2">
											<span class="font-medium text-white text-sm lg:text-base">{player.name}</span>
											{#if player.isHost}
												<div class="w-3 h-3 lg:w-4 lg:h-4 text-yellow-400">{@html CrownIcon}</div>
											{/if}
										</div>
										<div class="text-xs text-slate-400">
				...
				<div class="mb-4 lg:mb-6 p-3 lg:p-4 bg-white/10 rounded-lg border border-white/20">
					<h4 class="font-medium text-white mb-2 text-sm">Game Settings</h4>
					<div class="grid grid-cols-2 gap-3 text-xs lg:text-sm">
						<div class="flex items-center space-x-2">
							<div class="w-3 h-3 lg:w-4 lg:h-4 text-blue-400">{@html GlobeIcon}</div>
							<span class="text-slate-300">
								{languages[gameRoom?.settings?.language || 'english']?.name || 'English'}
							</span>
						</div>
						<div class="flex items-center space-x-2">
				...
				<div class="mt-auto">
					{#if currentPlayer?.isHost}
						<div class="space-y-3">
							<button
								...
								class="w-full bg-gradient-to-r from-green-600 to-teal-600 hover:from-green-500 hover:to-teal-500 disabled:from-gray-600 disabled:to-gray-600 text-white font-bold py-3 lg:py-4 px-4 lg:px-6 rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed text-sm lg:text-base flex items-center justify-center space-x-2"
							>
								...
								{:else}
									<div class="w-4 h-4 lg:w-5 lg:h-5">{@html PlayIcon}</div>
									<span>Start Game</span>
								{/if}
							</button>

							{#if totalPlayers < 3}
								<p class="text-center text-xs lg:text-sm text-red-400 font-medium">
					...
					{:else}
						<div
							class="text-center p-4 lg:p-6 bg-yellow-500/20 rounded-lg border border-yellow-500/30"
						>
							<div class="text-2xl lg:text-4xl mb-2 lg:mb-3">⏳</div>
							<p class="text-yellow-200 font-medium text-sm lg:text-base">
								Waiting for host to start
							</p>
							<p class="text-yellow-300/70 text-xs lg:text-sm mt-1">
								The host will start when ready
							</p>
						</div>
					{/if}
					...
					<div class="mt-4">
						<button
							on:click={resetGame}
							class="w-full bg-white/10 hover:bg-white/20 text-white font-medium py-2 lg:py-3 px-4 rounded-lg transition-all text-sm lg:text-base flex items-center justify-center space-x-2 hover:cursor-pointer"
						>
							<div class="w-4 h-4">{@html ArrowLeftIcon}</div>
			...
			<div class="max-w-md mx-auto">
				<div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
					...
					<div class="text-center mb-8">
						<div
							class="w-20 h-20 bg-gradient-to-br from-red-500 to-pink-600 rounded-2xl mx-auto mb-4 flex items-center justify-center shadow-xl"
						>
					...
					<div class="mb-8">
						<div
							class="text-center p-8 bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl shadow-xl"
						>
					...
					<div class="mb-6">
						<h3 class="text-lg font-bold text-gray-900 mb-4">
							Players ({playersRevealed}/{totalPlayers} revealed)
						</h3>
		...
		<div class="container mx-auto px-4 py-8 md:py-16">
			<div class="max-w-md mx-auto">
				<div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
					...
					<div class="text-center mb-8">
						<div
							class="w-20 h-20 bg-gradient-to-br from-yellow-500 to-orange-600 rounded-2xl mx-auto mb-4 flex items-center justify-center shadow-xl"
						>
					...
					<div class="mb-8 p-6 bg-blue-50 rounded-xl border border-blue-200">
						...
						<div class="space-y-2 text-sm text-blue-800">
							{#if imposterPlayer}
								<p>
									<strong>{imposterPlayer.name}</strong> was the imposter with the word "<strong
										>{imposterPlayer.word}</strong
									>"
								</p>
							{/if}
							{#if dumbPlayer}
